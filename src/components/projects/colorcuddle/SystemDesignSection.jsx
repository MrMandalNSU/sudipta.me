...
const userFlowSteps = [
  { key: "interaction", label: "User Input Gestures", sub: "Drag Marble / Click Orb" },
  { key: "orchestrator", label: "State Hub Dispatch", sub: "Update Guesses Grid state" },
  { key: "engine", label: "Permutation Evaluation", sub: "Calculate Clue Indicators" },
  { key: "console", label: "Stdout Console Logs", sub: "Render Clue Counters" },
  { key: "persistence", label: "Local Storage Stats", sub: "Update Streaks & Wins" },
];

const shareFlowSteps = [
  { key: "orchestrator", label: "Trigger Share Decryption", sub: "Won / Lost Overlay Modal" },
  { key: "share", label: "Difficulty Switched Parsing", desc: "Easy Mode Blocks vs Hard/Insane Mode Numeric logs" },
  { key: "share", label: "Clipboard Copy Action", sub: "ToastBanner alert trigger" },
];
...
const SystemDesignSection = ({
  ...
  const [activeMobileFlow, setActiveMobileFlow] = useState("gameplay");

  useEffect(() => {
    if (activeSystemNode === "share") {
      setActiveMobileFlow("share");
    } else if (
      activeSystemNode === "orchestrator" ||
      activeSystemNode === "interaction" ||
      activeSystemNode === "engine" ||
      activeSystemNode === "console" ||
      activeSystemNode === "persistence"
    ) {
      setActiveMobileFlow("gameplay");
    }
  }, [activeSystemNode]);
  ...
  return (
    ...
    >
      ...
      <Typography variant="body2" color="text.secondary" sx={{ mb: 3, maxWidth: 700 }}>
        An overview of Color Cuddle's reactive client-side logic loop, interaction handlers, and persistence caches.
        Click any node to see its role in the game execution stack.
      </Typography>
      ...
      <DiagramBoard sx={{ mb: 3, order: { xs: 2, md: 3 } }}>
        ...
        <Box sx={{ display: { xs: "none", md: "block" } }}>
          ...
          >
            ...
            {/* Connection Lines */}
            {[
              { from: "interaction", to: "orchestrator", path: "M 160 65 L 212 65" },
              { from: "orchestrator", to: "orchestrator", path: "M 285 90 L 285 122" },
              { from: "orchestrator", to: "engine", path: "M 350 155 L 370 155 L 370 65 L 382 65" },
              { from: "engine", to: "engine", path: "M 510 65 L 572 65" },
              { from: "engine", to: "persistence", path: "M 710 65 L 750 65 L 750 155 L 782 155" },
              { from: "engine", to: "console", path: "M 450 105 L 450 232" },
              { from: "orchestrator", to: "share", path: "M 285 180 L 285 205 L 100 205 L 100 232" },
              { from: "share", to: "share", path: "M 160 265 L 192 265" },
              { from: "share", to: "console", path: "M 320 265 L 372 265" },
              { from: "persistence", to: "orchestrator", path: "M 840 120 L 840 15 L 285 15 L 285 32" },
            ].map((line, lIdx) => {
              ...
              const dashStyle = line.from === "persistence" || line.to === "persistence" ? "5,3" : undefined;
              return (
                <path
                  key={lIdx}
                  d={line.path}
            ...
            {/* Nodes */}
            {[
              { key: "interaction", type: "start", x: 40, y: 40, w: 120, h: 50, rx: 25, cx: 100, cy: 65, label: "User Input Gestures", sub: "Drag / Click Orb" },
              { key: "orchestrator", type: "process", x: 220, y: 40, w: 130, h: 50, rx: 8, cx: 285, cy: 65, label: "State Hub", sub: "Props & State dispatch" },
              { key: "engine", type: "decision", d: "M 450 25 L 510 65 L 450 105 L 390 65 Z", cx: 450, cy: 65, label: "Confirm?", sub: "Compare Sequence" },
              { key: "engine", type: "process", x: 580, y: 40, w: 130, h: 50, rx: 8, cx: 645, cy: 65, label: "Evaluation", sub: "Compute Clue borders" },
              { key: "orchestrator", type: "process", x: 220, y: 130, w: 130, h: 50, rx: 8, cx: 285, cy: 155, label: "Game Loop Init", sub: "Reset rows & pools" },
              { key: "persistence", type: "cylinder", x: 790, y: 120, w: 100, h: 70, cx: 840, cy: 155, label: "Local Storage", sub: "Streak Persistence" },
              { key: "share", type: "process", x: 40, y: 240, w: 120, h: 50, rx: 8, cx: 100, cy: 265, label: "Trigger Share", sub: "Won / Lost Overlay" },
              { key: "share", type: "process", x: 200, y: 240, w: 120, h: 50, rx: 8, cx: 260, cy: 265, label: "Share Parser", sub: "Difficulty Emoji compile" },
              { key: "console", type: "process", x: 380, y: 240, w: 130, h: 50, rx: 8, cx: 445, cy: 265, label: "Console logs (stdout)", sub: "Print evaluations" },
            ].map((node, nIdx) => {
            ...
            <text x="20" y="340" fontSize="10.5" fill={theme.palette.text.secondary} fontFamily="Inter, sans-serif">
              ── Dispatch Pipeline  ╌╌ Persistence Sync  Click nodes to highlight execution pathways
            </text>
          </svg>
        </Box>
        ...
        <Box sx={{ display: { xs: "block", md: "none" } }}>
          ...
          >
            <Button
              fullWidth
              onClick={() => setActiveMobileFlow("gameplay")}
              size="small"
              sx={{
                textTransform: "none",
                fontWeight: activeMobileFlow === "gameplay" ? 800 : 600,
                fontSize: "0.8rem",
                borderRadius: 1.5,
                py: 0.8,
                backgroundColor: activeMobileFlow === "gameplay" ? (theme.palette.mode === "light" ? "#FFF" : "rgba(255,255,255,0.08)") : "transparent",
                color: activeMobileFlow === "gameplay" ? "primary.main" : "text.secondary",
                boxShadow: activeMobileFlow === "gameplay" && theme.palette.mode === "light" ? "0 2px 6px rgba(0,0,0,0.05)" : "none",
                border: "none",
                "&:hover": {
                  backgroundColor: activeMobileFlow === "gameplay" ? (theme.palette.mode === "light" ? "#FFF" : "rgba(255,255,255,0.08)") : "rgba(0,0,0,0.02)",
                }
              }}
            >
              Gameplay Loop Flow
            ...
            <Button
              fullWidth
              onClick={() => setActiveMobileFlow("share")}
              size="small"
              sx={{
                textTransform: "none",
                fontWeight: activeMobileFlow === "share" ? 800 : 600,
                fontSize: "0.8rem",
                borderRadius: 1.5,
                py: 0.8,
                backgroundColor: activeMobileFlow === "share" ? (theme.palette.mode === "light" ? "#FFF" : "rgba(255,255,255,0.08)") : "transparent",
                color: activeMobileFlow === "share" ? "primary.main" : "text.secondary",
                boxShadow: activeMobileFlow === "share" && theme.palette.mode === "light" ? "0 2px 6px rgba(0,0,0,0.05)" : "none",
                border: "none",
                "&:hover": {
                  backgroundColor: activeMobileFlow === "share" ? (theme.palette.mode === "light" ? "#FFF" : "rgba(255,255,255,0.08)") : "rgba(0,0,0,0.02)",
                }
              }}
            >
              Share Parser Flow
          ...
          <Box sx={{ display: "flex", flexDirection: "column", gap: 3, pl: 1, pr: 1 }}>
            {(activeMobileFlow === "gameplay" ? userFlowSteps : shareFlowSteps).map((step, idx, arr) => {
